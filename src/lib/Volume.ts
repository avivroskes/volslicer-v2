 ...
 * See the webgl_loader_nrrd.html example and the loaderNRRD.lib file to see how to use this class.
 * @class
...
export class Volume {
  ...
  reverseAccess(index) {
    const z = Math.floor(index / (this.yLength * this.xLength)),
      y = Math.floor((index - z * this.yLength * this.xLength) / this.xLength),
      x = index - z * this.yLength * this.xLength - y * this.xLength;
    return [x, y, z];
  ...
  map(functionToMap, context) {
    ...
    this.data.forEach((data, index, array) => {
      data = functionToMap.call(context, data, index, array);
    });
   ...
   * @memberof THREE.Volume
   * @param {axis} axis  the normal axis to the slice 'x' 'y' or 'z'
   * @param {RASIndex} index the index of the slice
  ...
  extractPerpendicularPlane(axis, RASIndex) {
    ...
    const planeMatrix = (new THREE.Matrix4()).identity(),
      volume = this,
      axisInIJK = new THREE.Vector3(),
      firstDirection = new THREE.Vector3(),
      secondDirection = new THREE.Vector3(),
      dimensions = new THREE.Vector3(this.xLength, this.yLength, this.zLength);
    ...
    const base = [new THREE.Vector3(1, 0, 0), new THREE.Vector3(0, 1, 0), new THREE.Vector3(0, 0, 1)],
      iDirection = [firstDirection, secondDirection, axisInIJK].find(function (x) {
        return Math.abs(x.dot(base[0])) > 0.9;

      }),
      jDirection = [firstDirection, secondDirection, axisInIJK].find(function (x) {
        return Math.abs(x.dot(base[1])) > 0.9;

      }),
      kDirection = [firstDirection, secondDirection, axisInIJK].find(function (x) {
