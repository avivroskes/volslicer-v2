...
export class MRCLoader {
  ...
  ,
    ...
    parse: function (data);
    {
      ...
      let _dimensions = [MRC.nx, MRC.ny, MRC.nz]; // voxel(i,j,k)
      volume.dimensions = _dimensions;

      // get voxel spacing
      let spacingX = MRC.xlen / MRC.mx;
      let spacingY = MRC.ylen / MRC.my;
      let spacingZ = MRC.zlen / MRC.mz;
      ...
      // (Set these value as transpose of required matrix--three.lib thing)
      ...
      /*
          ...
          let tar = goog.vec.Vec4.createFloat32FromValues(0, 0, 0, 1);
          let res = goog.vec.Vec4.createFloat32();
          goog.vec.Mat4.multVec4(IJKToRAS, tar, res);

          //  Transform ijk (spacingX, spacinY, spacingZ) to RAS
          let tar2 = goog.vec.Vec4.createFloat32FromValues(1, 1, 1, 1);
          let res2 = goog.vec.Vec4.createFloat32();
   ...
   parseStream(_data) {
    ...
    MRC.nx = scan('sint');
    console.log('nx = ' + MRC.nx);
    MRC.ny = scan('sint');
    console.log('ny = ' + MRC.ny);
    MRC.nz = scan('sint');
    console.log('nz = ' + MRC.nz);
    MRC.mode = scan('sint');

    _dataPointer = 28;

    MRC.mx = scan('sint');
    console.log('mx = ' + MRC.mx);
    MRC.my = scan('sint');
    console.log('my = ' + MRC.my);
    MRC.mz = scan('sint');
    console.log('mz = ' + MRC.mz);

    //  pixel spacing = xlem/mx
    MRC.xlen = scan('float');
    console.log('xlen = ' + MRC.xlen);
    MRC.ylen = scan('float');
    ...
    //  size of the image
    let volsize = MRC.nx * MRC.ny * MRC.nz;
   ...
   scan(type, chunks) {
    ...
    let _start = _dataPointer;
    let _end = _dataPointer += chunks * _chunkSize;

    console.log(_start + ',' + _end);

    let _data_slice = _data.slice(_start, _end);
  ...
  // Swapping the bits to match the endianness
   flipEndianness(array, chunkSize) {
